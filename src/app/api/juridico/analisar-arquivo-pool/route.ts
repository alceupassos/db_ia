...
Responda APENAS no seguinte formato JSON (sem markdown, sem texto adicional):
{
  ...
  "descricao": "Resumo descritivo do documento em 3-5 frases, incluindo informações extraídas do nome"
    ...
    // Atualiza o arquivo no pool com categoria e descrição
    ...
    if (updateError) {
      console.error('Erro ao atualizar arquivo do pool:', updateError);
      return NextResponse.json(
    ...
    console.error('Erro ao analisar arquivo do pool:', error);
    return NextResponse.json(
